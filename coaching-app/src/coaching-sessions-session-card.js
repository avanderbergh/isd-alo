...
class CoachingSessionsSessionCard extends PolymerElement {
  ...
  static get observers() {
    return ["_sessionChanged(session)"];
  }
            ...
            <template is="dom-if" if="{{!attending}}">
              ...
              <template is="dom-if" if="{{!sessionFull}}">
                <paper-icon-button
                  ...
                  on-tap="_handleAttendSessionTapped"
                ></paper-icon-button>
              </template>
            </template>
            <template is="dom-if" if="{{attending}}">
              <paper-icon-button
                icon="check-box"
                on-tap="_handleUnattendSessionTapped"
              ></paper-icon-button>
            </template>
          </template>
        </div>
  ...
  _computeDisplayTime(session) {
    ...
    const displayEnd = new Date(end.valueOf());
    const displayTime =
      format(start, "H:mm") + " - " + format(displayEnd, "H:mm");
    return displayTime;
  }
...
customElements.define(
  "coaching-sessions-session-card",
  CoachingSessionsSessionCard
);
