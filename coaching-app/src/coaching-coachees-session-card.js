...
class CoachingCoacheesSessionCard extends PolymerElement {
        ...
        return html`
            ...
            <div id="sessionCard">
                ...
                <div style="width: 20rem;">
                    <a href="/sessions/[[session.__id__]]">[[workshop.title]]: [[session.title]]</a>
                </div>
    ...
    _fetchWorkshop() {
        ...
        if (this.session) {
            if (this.session.hasOwnProperty('workshop')) {
                db.collection('workshops').doc(this.session.workshop).onSnapshot(doc => {
    ...
    _fetchSpace() {
        const db = firebase.firestore();
        db.collection('spaces').doc(this.session.space).get().then(doc => {
    ...
    _fetchAttended() {
        ...
        const db = firebase.firestore();
        db.collection('sessions').doc(this.session.__id__)
            .collection('attendance').doc(this.userId).onSnapshot(doc => {
...
customElements.define('coaching-coachees-session-card', CoachingCoacheesSessionCard);