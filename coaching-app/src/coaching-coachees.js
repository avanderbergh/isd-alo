...
class CoachingCoachees extends PolymerElement {
    ...
    static get template() {
        return html`
            ...
            <template is="dom-if" if="{{user.claims.student}}">
                <coaching-coachees-student-view user="[[user]]" days="[[days]]"></coaching-coachees-student-view>    
            </template>
            <template is="dom-if" if="{{user.claims.staff}}">
                <coaching-coachees-staff-view user="[[user]]" days="[[days]]"></coaching-coachees-staff-view>    
            </template>
        `;
    }
    ...
    ready() {
        ...
        db.collection('days').orderBy('startTime').onSnapshot(querySnapshot => {
            this.set('days', []);
