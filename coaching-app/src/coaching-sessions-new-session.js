        ...
        <div id="newSession">
          ...
          <paper-dropdown-menu label="Workshop">
            <paper-listbox
              ...
              attr-for-selected="workshop"
            >
              <template is="dom-repeat" items="{{workshops}}" as="workshop">
                <paper-item workshop="[[workshop.__id__]]"
                  >[[workshop.title]]</paper-item
                >
              </template>
              <paper-item on-tap="_handleNewWorkshopTapped">
                Create a new Workshop</paper-item
              >
            </paper-listbox>
          </paper-dropdown-menu>
          ...
          <div>
            <p>
              <span class="form-label"
                >Which grade levels can attend this session?</span
              >
            </p>
            <template is="dom-repeat" items="{{grades}}" as="grade">
              <paper-checkbox
                ...
                data-grade$="[[grade]]"
                on-checked-changed="_gradeChecked"
                >Grade [[grade]]</paper-checkbox
              >
            </template>
          </div>
          <div>
            <paper-dropdown-menu label="Start Time">
              ...
              >
                <template is="dom-repeat" items="{{startTimes}}" as="startTime">
                  <paper-item date="[[startTime.date]]"
                    >[[startTime.display]]</paper-item
                  >
                </template>
              </paper-listbox>
            </paper-dropdown-menu>
            <paper-dropdown-menu label="End Time">
              ...
              >
                <template is="dom-repeat" items="{{endTimes}}" as="endTime">
                  <paper-item date="[[endTime.date]]"
                    >[[endTime.display]]</paper-item
                  >
                </template>
              </paper-listbox>
            </paper-dropdown-menu>
          ...
          <div style="display: flex;">
            <paper-input
              ...
              style="width: 5rem; margin-right: 2rem;"
            ></paper-input>
            <paper-dropdown-menu label="Select a Space">
              <paper-listbox
                slot="dropdown-content"
                selected="{{formData.space}}"
                attr-for-selected="space"
              >
                <template is="dom-repeat" items="{{filteredSpaces}}" as="space">
                  <paper-item space="[[space.__id__]]"
                    >[[space.name]] ([[space.capacity]])</paper-item
                  >
                </template>
              </paper-listbox>
            </paper-dropdown-menu>
    ...
    db.collection("sessions")
      ...
      .then(querySnapshot => {
        ...
        querySnapshot.forEach(doc => {
          ...
          if (session.startTime.toDate() < endTime)
            usedSpaces.push(session.space);
        });
