...
class CoachingSessionAttendance extends PolymerElement {
        ...
        return html`
            ...
            <div class="attendees-list">
                <template is="dom-repeat" items="{{session.attendees}}" as="attendee">
                    <div class="attendee">
                        <coaching-attendance-button user-id="[[attendee]]" session-id="[[session.__id__]]"></coaching-attendance-button>
                    </div>
                </template>
            </div>
        `;
    ...
    _handleAttendedTapped(e) {
        ...
        db.collection('sessions').doc(this.session.__id__).update(attendanceUpdate);
    }
}
